...
def on_ask(question):
    ...
    try:
        ...
        # Сервис на VPS доступен всегда, а модели живут на домашней машине
        # и поднимаются по запросу. Для посетителя это штатное состояние,
        # а не поломка — важно, чтобы текст читался именно так.
        return ("**Модели сейчас не запущены.** Они работают на домашней машине "
                "с видеокартой и включаются по запросу — держать их в памяти "
                "круглосуточно ради демонстрации незачем. Устройство проекта, "
                "замеры и описание развёртывания — "
                "в [репозитории](https://github.com/shaone90/rfc-rag)."), ""
